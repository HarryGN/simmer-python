...
def check_ultrasonic(sensor_id: str):
    '''
    Check the ultrasonic sensor for a given sensor_id and handle the response.
    '''
    ...
    if packet_tx:
        ...
        print(f"Ultrasonic {sensor_id} response: {response_string(sensor_id, responses)}")
        
        if responses and responses[0] and len(responses[0]) > 1:
            try:
                return float(responses[0][1])
            except ValueError:
                print(f"Invalid float conversion for sensor {sensor_id}: {responses[0][1]}")
                return None
        else:
            print(f"Invalid response received for {sensor_id}: {responses[0]}")
            return None
    return None
...
while RUN_DEAD_RECKONING:
    ...
    # Continuously check ultrasonic sensors 'u0', 'u1', 'u2', 'u3'
    u0response = check_ultrasonic('u0')
    u1response = check_ultrasonic('u1')
    u2response = check_ultrasonic('u2')
    u3response = check_ultrasonic('u3')

    # Print the sensor readings for debugging
    print(f"Ultrasonic 0 response: {u0response}")
    print(f"Ultrasonic 1 response: {u1response}")
    print(f"Ultrasonic 2 response: {u2response}")
    print(f"Ultrasonic 3 response: {u3response}")

    # Check if any of the ultrasonic responses is <= 2 (collision)
    if u0response is not None and u1response is not None and u2response is not None and u3response is not None:
        if u0response <= 2 or u1response <= 2 or u2response <= 2 or u3response <= 2:
            transmit('xx')
            print("2 inches from collision, stopping")
            break
