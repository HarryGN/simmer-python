...
def extract_numeric_value(response_string: str) -> float:
    """
    Extracts a numeric value from a response string in the format:
    'cmd u3 = u3 ✓, response "26.285"'
    
    Args:
        response_string (str): The string containing the response.
    
    Returns:
        float: The extracted numeric value, or None if no valid value is found.
    """
    # Regular expression pattern to extract the numeric value inside quotes
        ...
        for sensor_id in ['u0', 'u1', 'u2', 'u3']:
                ...
                if packet_tx:
                    ...
                    try:
                        ...
                        pass
                    # Check if responses is a list and has expected data
                    measured_distance = response_string(sensor_id, responses)
                    
                    # Check if response_value is of expected typ
                    reading = extract_numeric_value(measured_distance)
                    print(f"Ultrasonic {sensor_id} reading: {reading}")
                    try:
                        if reading <= OBSTACLE_THRESHOLD:
                                print("need to stop rover")
                                packet_tx = packetize('xx')
                                if packet_tx:
                                    transmit(packet_tx)
                                    [responses, time_rx] = receive()
                                    print(f"stop command response: {response_string(CMD_SEQUENCE[ct],responses)}")
