            ...
            if any(value is not None for value in direction_map.values()):
                ...
                for direction, reading in direction_map.items():
                    if reading == -1:
                        ...
                        print(f"{direction.capitalize()}: {reading} cm")
            ...
            if len(sensor_info) == 2:
                try:
                    ...
                    if distance_text.isdigit():
                        ...
                        distance_cm = -1 if distance == 8191 else distance / 10  # Convert to cm
                    else:
                        # Handle non-numeric values (e.g., "Out of range")
                        distance_cm = -1  # Use -1 to represent "Out of range"
...
def make_decision(ser):
    ...
    while count < 5:
        ...
        # Case 4: Diagonal obstacle on left
            ...
            cmd_list.extend(["w0: -2", "r0: 10", "w0: 1", "r0: -10"])
            count += 1
        ...
        # Case 6: Clear path, move forward and correct path
            ...
            if (left < 55 or diag_left < 78) and right >= 55:
                ...
                cmd_list.extend(["STOP", "r0: 5", "w0: 1", "r0: -5"])
                ...
                if abs(left - right) > 0.5:
                    if left > right:
                        ...
                        cmd_list.extend(["r0: -5", "w0: 1", "r0: 5"])
                    else:
            ...
            transmit(packetize("w0: 2"))  # Move forward
            cmd_list.append("w0: 2")
            count += 1
...
try:
    ...
    left_reading = direction_map['left']
    left_front_reading = direction_map['left-front']
    front_reading = direction_map['front']
    right_front_reading = direction_map['right-front']
    right_reading = direction_map['right']
        
        #  # Print each individual reading
        # print("\nIndividual ToF Distance Readings:")
        # print(f"Left: {left_reading if left_reading != -1 else 'Out of range'} mm")
        # print(f"Left-front: {left_front_reading if left_front_reading != -1 else 'Out of range'} mm")
        # print(f"Front: {front_reading if front_reading != -1 else 'Out of range'} mm")
        # print(f"Right-front: {right_front_reading if right_front_reading != -1 else 'Out of range'} mm")
        # print(f"Right: {right_reading if right_reading != -1 else 'Out of range'} mm")
